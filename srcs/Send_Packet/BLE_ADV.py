...
class BLE_ADV():
    def __init__(self, advertiser_address:str,master_address:str):
        self.name = "adv_pkts"
    ...
    def ADV_NONCONN_IND(self):
        ...
        return pkt
    def SCAN_REQ(self):
        pkt = BTLE() / BTLE_ADV(RxAdd = 0 ,TxAdd = 1)/ BTLE_SCAN_REQ(ScanA=self.master_address,AdvA=self.advertiser_address)
        return pkt
    def SCAN_RSP(self):
        pkt = BTLE() / BTLE_ADV()/ BTLE_SCAN_RSP(AdvA=self.advertiser_address)
        return pkt
    def CONNECT_REQ(self):
        pkt = BTLE() / BTLE_ADV( RxAdd = 0 ,TxAdd = 1)/ BTLE_CONNECT_REQ(InitA = self.master_address,AdvA=self.advertiser_address,crc_init = 0x179a9c,win_size = 2,\
                                                    win_offset = 1, interval = 16, latency = 0, timeout = 50, chM= 0x1FFFFFFFFF,\
                                                        hop = 5,SCA = 0)
        return pkt
