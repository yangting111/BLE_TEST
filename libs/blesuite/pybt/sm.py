...
class SM:
    ...
    def __init__(self, io_cap=0x03, oob=0x0, mitm=0x00, bond=0x01, lesc=0x0, keypress=0x0, ct2=0x01, rfu=0x0,
        ...
        self.peer_initiator_key_distribution = None
        self.peer_responder_key_distribution = None
        ...
        self.smp_pairing_request = None
        self.smp_pairing_response = None
        self.smp_pairing_confirm = None
        self.smp_pairing_random = None
        self.smp_pairing_failed = None
        self.smp_encryption_information = None
        self.smp_master_identification = None
        self.smp_identity_information = None
        self.smp_identity_address_information = None
        self.smp_signing_information = None
        self.smp_security_request = None
        self.smp_pairing_public_key = None
        self.smp_pairing_dhkey_check = None
        self.smp_pairing_keypress_notification = None
    ...
    def calculate_confirm(self, master=0):
        ...
        return bt_crypto_c1(self.tk, rand, self.prsp, self.preq, self.ia_type, self.ia, self.ra_type, self.ra)
    ...
    def get_packet(self, packet_type):
        if packet_type == SMP_PAIRING_REQUEST:
            return self.smp_pairing_request
        elif packet_type == SMP_PAIRING_RESPONSE:
            return self.smp_pairing_response
        elif packet_type == SMP_PAIRING_CONFIRM:
            return self.smp_pairing_confirm
        elif packet_type == SMP_PAIRING_RANDOM:
            return self.smp_pairing_random
        elif packet_type == SMP_PAIRING_FAILED:
            return self.smp_pairing_failed
        elif packet_type == SMP_ENCRYPTION_INFORMATION:
            return self.smp_encryption_information
        elif packet_type == SMP_MASTER_IDENTIFICATION:
            return self.smp_master_identification
        elif packet_type == SMP_IDENTITY_INFORMATION:
            return self.smp_identity_information
        elif packet_type == SMP_IDENTITY_ADDRESS_INFORMATION:
            return self.smp_identity_address_information
        elif packet_type == SMP_SIGNING_INFORMATION:
            return self.smp_signing_information
        elif packet_type == SMP_SECURITY_REQUEST:
            return self.smp_security_request
        elif packet_type == SMP_PAIRING_PUBLIC_KEY:
            return self.smp_pairing_public_key
        elif packet_type == SMP_PAIRING_DHKEY_CHECK:
            return self.smp_pairing_dhkey_check
        elif packet_type == SMP_PAIRING_KEYPRESS_NOTIFICATION:
            return self.smp_pairing_keypress_notification
        else:
            return None


def u128_xor(a1:bytes, a2:bytes):
    result = a1 ^ a2
...
class SecurityManagerProtocol:
    ...
    def __init__(self, long_term_key_db=None, default_io_cap=0x03, default_oob=0x0,
                 default_mitm=0x00, default_bond=0x01,
                 default_lesc=0x00, default_keypress=0x00, default_ct2=0x01, default_rfu=0x00,
                 default_max_key_size=16,
                 default_initiator_key_distribution=0x01, default_responder_key_distribution=0x01):
        # self.stack = stack
        self.security_managers: Dict[str, SM] = {}  # hold security manager for each connection handle
    ...
    def marshall_command(self, connection_handle, packet:Packet, peer_address):
        ...
        if code == SMP_PAIRING_REQUEST:
            ...
            sm.smp_pairing_response = packet
            sm.prsp = raw(packet[SM_Hdr])[::-1]
            ...
            sm.smp_pairing_confirm = SM_Hdr()/ SM_Confirm(confirm=confirm_value[::-1])
    ...
    def initiate_encryption_with_existing_keys(self, address, address_type,
                                               our_address, our_address_type, our_role):
    ...
    def send_distribution_keys(self, peer_address, connection_handle):
        ...
        if ltk:
            ...
            # send ltk
            sm.smp_encryption_information = SM_Hdr()/SM_Encryption_Information(ltk=sm.ltk[::-1])
            ...
            sm.smp_master_identification = SM_Hdr()/SM_Master_Identification(ediv=sm.ediv, rand=sm.randomVal[::-1])
            #self.send(SM_Master_Identification(ediv=sm.ediv, rand=sm.randomVal[::-1]), connection_handle)
        if irk:
            log.debug("Sending IRK")
            # send irk
            sm.smp_identity_information = SM_Hdr()/SM_Identity_Information(irk=sm.irk[::-1])
            ...
            if sm.our_role == ROLE_TYPE_PERIPHERAL:
                sm.smp_identity_address_information = SM_Hdr()/SM_Identity_Address_Information(atype=sm.ra_type,
                                                                                               addr=sm.ra)
                #self.send(SM_Identity_Address_Information(atype=sm.ra_type, addr=sm.ra), connection_handle)
            else:
                sm.smp_identity_address_information = SM_Hdr()/SM_Identity_Address_Information(atype=sm.ia_type,
                                                                                               addr=sm.ia)
                #self.send(SM_Identity_Address_Information(atype=sm.ia_type, addr=sm.ia), connection_handle)
        if csrk:
            log.debug("Sending CSRK")
            # send csrk
            sm.smp_signing_information = SM_Hdr()/SM_Signing_Information(csrk=sm.csrk[::-1])
