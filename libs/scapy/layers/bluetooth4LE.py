...
class BTLE_CTRL(Packet):
    ...
    def dissection_done(self, pkt):
        if self.opcode == 0x05:
            self.add_payload(LL_START_ENC_REQ())
        elif self.opcode == 0x06:
            self.add_payload(LL_START_ENC_RSP())
        elif self.opcode == 0x0a:
            self.add_payload(LL_PAUSE_ENC_REQ())
        elif self.opcode == 0x0b:
            self.add_payload(LL_PAUSE_ENC_RSP())
        elif self.opcode == 0x12:
            self.add_payload(LL_PING_REQ())
        elif self.opcode == 0x13:
            self.add_payload(LL_PING_RSP())
...
bind_layers(BTLE_CTRL, LL_20, opcode=0x20)
