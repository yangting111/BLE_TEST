...
class L2CAP_Hdr(Packet):
    ...
    fields_desc = [LEShortField("len", None),
                   LEShortEnumField("cid", 0, {1: "control", 4: "attribute",5:"l2cap signaling"}), ]  # noqa: E501
...
class L2CAP_Create_Channel_Request(Packet):
    ...
    fields_desc = [LEShortField("psm", 0),
                   LEShortField("source_cid", 0), 
                   LEShortField("controller_id", 0),]
    
class L2CAP_Create_Channel_Response(Packet):
    name = "L2CAP Create Channel Response"
    fields_desc = [LEShortField("destination_cid", 0),
                   LEShortField("source_cid", 0),
...
class L2CAP_Move_Channel_Request(Packet):
    ...
    fields_desc = [LEShortField("initiator_cid", 0),
                   LEShortField("destination_cid", 0) ]
...
class L2CAP_LE_Flow_Control_Credit_IND(Packet):
    name = "L2CAP LE Flow Control Credit"
    fields_desc = [
        LEShortField("cid", 0),
        LEShortField("credit", 0), ]
...
class ATT_Write_Command(Packet):
    name = "Write Request"
    fields_desc = [XLEShortField("gatt_handle", 0),
                   ConditionalField(StrField("data", ""), lambda pkt: pkt.gatt_handle != 0x0030 and pkt.gatt_handle != 0x002a)]
...
class ATT_Handle_Value_Notification(Packet):
    ...
    fields_desc = [XLEShortField("gatt_handle", 0),
                   ConditionalField(StrField("value", ""), lambda pkt: pkt.gatt_handle != 0x0032 and pkt.gatt_handle != 0x002c)]
...
class SM_Security_Request(Packet):
    ...
    fields_desc = [ByteField("auth_req", 0)]
...
class SM_Keypress_Notification(Packet):
    ...
    fields_desc = [ByteEnumField("notification_type", 0, {0: "Passkey Entry Started", 1: "Passkey Digit Entered", 2: "Passkey Digit Erased", 3: "Passkey Cleared", 4: "Passkey Entry Completed"}),  # noqa: E501
                   ]
...
bind_layers(L2CAP_CmdHdr, L2CAP_LE_Flow_Control_Credit_IND, code=22)
