...
class FuzzLoggerDb(ifuzz_logger_backend.IFuzzLoggerBackend):
    ...
    def __init__(self, db_filename, num_log_cases=0):
        ...
        if not db_already_exist:
            self._db_cursor.execute("""CREATE TABLE cases (name text, raw packet, timestamp TEXT)""")
            self._db_cursor.execute(
                """CREATE TABLE steps (type, pktname, path, raw, timestamp)"""
            )
    ...
    def get_test_case_data(self, index):
        ...
        try:
            ...
            return None
        rows = c.execute("""SELECT * FROM steps WHERE type=?""", [index])
        steps = []
        for row in rows:
            data = row[3]
    ...
    def open_test_case(self, name, cur_path):
        cur_path_str = ''.join([str(x) for x in cur_path])
        self._queue.append(["INSERT INTO cases VALUES(?, ?, ?);\n", name, cur_path_str, helpers.get_time_stamp()])
        self._current_test_case_index = name
    
    def log_test_case(self, name, rawpkt):
        self._queue.append(["INSERT INTO cases VALUES(?, ?, ?);\n", name, rawpkt, helpers.get_time_stamp()])
    ...
    def log_error(self, pktname, path, rawpkt):
