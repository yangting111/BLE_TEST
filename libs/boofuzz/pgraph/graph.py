...
class Graph:
    ...
    def add_edge(self, graph_edge, prevent_dups=True):
        ...
        if self.find_node("label", graph_edge.src.label) is not None and self.find_node("label", graph_edge.dst.label) is not None:
    ...
    def edges_from(self, edge_label):
        """
        ...
        return [edge_value for edge_value in list(self.edges.values()) if edge_value.src.label == edge_label]
    ...
    # Perform depth-first search on a given graph and print the node id 
    ...
    def dfs(self, node, visited):
        ...
        for edge in self.edges_from(node.label):
            
            if edge.dst.label not in visited:
                next_node = self.find_node("label", edge.dst.label)
                if next_node is None:
                    raise Exception("No next nod found from {0}".format(node.label))
                # yield next_node.label
                # yield from self.dfs( next_node, visited)
                self.dfs( next_node, visited)

        self.pathlist.append(visited.copy())

        visited.pop()

    def graph_path(self, from_node):
        """
